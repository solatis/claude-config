...
| ------------------- | ----------------------------------------- | ---------------------------------------------- |
| `user.rb`           | User model, scopes, validations           | Adding user attributes, modifying auth logic   |
| `application_controller.rb` | Base controller, auth filters   | Adding global before_action, error handling    |

## Subdirectories

| Directory    | What                            | When to read                                |
| ------------ | ------------------------------- | ------------------------------------------- |
| `models/`    | ActiveRecord models and scopes  | Adding associations, writing queries        |
| `services/`  | Business logic service objects  | Adding features, modifying business rules   |
```
...
| Directory      | What                                          | When to read        |
| -------------- | --------------------------------------------- | ------------------- |
| `db/schema.rb` | Generated by `rails db:migrate`. Never edit   | Never edit directly |
| `vendor/`      | Vendored gems, upstream: Gemfile.lock         | Never edit directly |
| `app/assets/builds/` | Compiled assets from `yarn build`       | Never edit directly |
...
```typescript
// Polling: 30% webhook delivery failures observed in production
const result = await pollEndpoint(url, { interval: 30 })
```
...
```typescript
function reconcileState(local: State, remote: State): void {
  // Reconciles local state against remote source of truth. Operates in
  // three phases:
  // 1. Diff local vs remote to find divergent keys
  // 2. For each divergence, apply conflict resolution (remote wins)
...
```ruby
# Strip whitespace and downcase. Use before cache lookup.
def normalize_key(k)
```
...
#
# @param path [String] Absolute path to YAML config file.
# @param strict [Boolean] Treat unknown keys as errors.
# @return [Config] Validated Config object.
#
# @example
 ...
 * @param strict - Treat unknown keys as errors.
 * @returns Validated Config object.
 *
 * @example
 *   const cfg = validateConfig("/etc/app/config.yaml", { strict: true })
 */
function validateConfig(path: string, strict = false): Config {
```
